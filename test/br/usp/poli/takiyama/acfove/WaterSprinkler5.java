...
public class WaterSprinkler5 {
	...
	public void querySprinklerGivenWetGrass() {
		...
		Factor afterSumOutSprinkler = afterSumOutWetGrass.multiply(gs.factor()).sumOut(sprinkler);
		
		Prv cloudy = network.prv("cloudy ( )");
		Factor afterSumOutCloudy = afterSumOutSprinkler.multiply(gc.factor()).multiply(gr.factor()).sumOut(cloudy);
